...
def api_coletar_projeto():
    ...
    for ap in rows:
        ...
        horas = max(0, (agora_dt - dt).total_seconds() / 3600)
        taxa_dia = float(ap['rendimento_diario'])
        rend = round(float(ap['valor']) * taxa_dia * (horas / 24), 6)

        # Coleta qualquer rendimento positivo que já aparece no painel.
        if rend <= 0:
            continue
        ...
        conn.execute('UPDATE aportes_projeto SET ultimo_collect=? WHERE id=?', (agora, ap['id']))
        conn.execute(
            'INSERT INTO rendimentos (usuario,projeto_id,valor,percentual,saldo_base) VALUES(?,?,?,?,?)',
            (session['user'], ap['projeto_id'], rend, taxa_dia * (horas / 24), float(ap['valor']))
        )
        conn.execute(
