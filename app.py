...
def api_dashboard():
    conn = get_db()
    num_contas = conn.execute(
        'SELECT COUNT(*) FROM contas WHERE usuario=?',
        (session['user'],)
    ).fetchone()[0]

    num_posts = conn.execute(
        'SELECT COUNT(*) FROM agendamentos WHERE conta IN (SELECT nome FROM contas WHERE usuario=?)',
        (session['user'],)
    ).fetchone()[0]

    # A tabela campanhas nao possui coluna `conta`, entao o total precisa ser calculado sem esse filtro.
    orcamento = conn.execute(
        'SELECT COALESCE(SUM(custo),0) FROM campanhas'
    ).fetchone()[0]

    # Quantidade total de tarefas pendentes no marketplace.
    num_tarefas = conn.execute(
        "SELECT COUNT(*) FROM tarefas WHERE status='pendente' AND trabalhador IS NULL"
    ).fetchone()[0]
...
def api_tarefas():
    ...
    if request.method == 'POST':
        ...
        if conta_existe == 0:
            conn.close()
            return jsonify({"error": "A conta selecionada nao pertence ao usuario logado"}), 403
...
def tarefas_disponiveis():
    conn = get_db()
    rows = conn.execute(
        "SELECT id,tipo,quantidade,conta,nicho,recompensa,valor_total,contratante FROM tarefas WHERE trabalhador IS NULL AND status='pendente' AND contratante != ? ORDER BY data_criacao DESC",
        (session['user'],)
    ).fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
