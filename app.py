...
def api_me():
    ...
    for ap in aportes:
        ...
        rendimento_pendente += max(0, round(float(ap['valor']) * float(ap['rendimento_diario']) * (horas/24), 6))

    saldo_em_projetos = conn.execute(
        "SELECT COALESCE(SUM(valor),0) FROM aportes_projeto WHERE usuario=? AND status='aprovado'",
        (session['user'],)
    ).fetchone()[0]
        ...
        if d['status'] == 'aprovado' and d['projeto_status'] == 'ativo':
            ...
            if ultimo:
                try:
                    ...
                    horas = (datetime.now() - dt).total_seconds() / 3600
                    d['rendimento_pendente'] = round(max(0, float(d['valor']) * float(d['rendimento_diario']) * (horas/24)), 6)
...
def api_coletar_projeto():
    ...
    ap = conn.execute(
        ...
        "JOIN projetos p ON p.id=ap.projeto_id WHERE ap.id=? AND ap.usuario=? AND ap.status='aprovado'",
        (aporte_id, session['user'])
    ).fetchone()
    if not ap:
        conn.close(); return jsonify({'error': 'Aporte não encontrado'}), 404
    if ap['projeto_status'] != 'ativo':
        conn.close(); return jsonify({'error': 'Projeto não está ativo'}), 400
    ultimo = ap['ultimo_collect'] or ap['data_aprovacao']
    if not ultimo:
        conn.close(); return jsonify({'error': 'Nenhum aporte aprovado'}), 400
    try: dt = datetime.fromisoformat(ultimo)
    except: dt = datetime.now() - timedelta(hours=2)
    horas = (datetime.now() - dt).total_seconds() / 3600
    if horas < 1:
        conn.close(); return jsonify({'error': f'Aguarde {int((1-horas)*60)+1} min para coletar.'}), 400
    taxa = float(ap['rendimento_diario'])
    rend = round(float(ap['valor']) * taxa * (horas/24), 6)
    if rend <= 0:
        conn.close(); return jsonify({'error': 'Rendimento ainda não disponível'}), 400
    agora = datetime.now().isoformat()
    conn.execute('UPDATE aportes_projeto SET ultimo_collect=? WHERE id=?', (agora, aporte_id))
    conn.execute('UPDATE users SET saldo_disponivel=saldo_disponivel+?,total_ganho=total_ganho+? WHERE username=?',
                 (rend, rend, session['user']))
    conn.execute('INSERT INTO rendimentos (usuario,projeto_id,valor,percentual,saldo_base) VALUES(?,?,?,?,?)',
                 (session['user'], ap['projeto_id'], rend, taxa*(horas/24), float(ap['valor'])))
    conn.execute('INSERT INTO movimentacoes (usuario,tipo,valor,descricao) VALUES(?,?,?,?)',
                 (session['user'],'RENDIMENTO',rend,f'Rendimento projeto #{ap["projeto_id"]} — {round(taxa*100,2)}%/dia'))
    conn.commit(); conn.close()
