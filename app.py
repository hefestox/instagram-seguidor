...
def tarefas_disponiveis():
    ...
    if not row['subscribed']:
        ...
        return jsonify({
            "blocked": True,
            "message": "Assine o NichoPost Pro para trabalhar e pegar tarefas.",
            "tasks": []
        }), 403
...
def pegar_tarefa(tarefa_id):
    ...
    if not row['subscribed']:
        conn.close()
        return jsonify({"error": "Assine o NichoPost Pro para trabalhar"}), 403
...
def verificar_tarefa(tarefa_id):
    ...
    conn = get_db()
    row = conn.execute('SELECT subscribed FROM users WHERE username=?', (session['user'],)).fetchone()
