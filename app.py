...
def registrar_comissao(conn, referrer, referred, origem, base_valor, percentual, valor, ref_tabela, ref_id):
    ...
    registrar_movimentacao(conn, referrer, 'COMISSAO_INDICACAO', valor, f'Comissão de {origem} do indicado {referred}', ref_tabela, ref_id)


def registrar_movimentacao(conn, usuario, tipo, valor, descricao='', ref_tabela='', ref_id=''):
    conn.execute(
        '''INSERT INTO movimentacoes
           (usuario,tipo,valor,descricao,ref_tabela,ref_id)
           VALUES(?,?,?,?,?,?)''',
        (usuario, tipo, valor, descricao, ref_tabela, str(ref_id or ''))
    )
...
def init_db():
    ...
    ensure_column(conn, 'saques', 'aprovado_por', 'TEXT')
    ensure_column(conn, 'saques', 'observacao', 'TEXT')
    ensure_column(conn, 'saques', 'data_processamento', 'DATETIME')
...
def assinar():
    ...
    registrar_movimentacao(conn, session['user'], 'ASSINATURA_PRO', -PLANO_PRECO, 'Assinatura do plano NichoPost Pro', 'users', row['id'])
    bonus = 0
    if row['indicado_por']:
...
def api_tarefas():
    ...
    if request.method == 'POST':
        ...
        registrar_movimentacao(conn, session['user'], 'CONTRATACAO_TAREFA', -valor_total, f'Contratação de {quantidade} {tipo}(s) para {conta}', 'tarefas', conta)
        conn.execute(
...
def tarefas_disponiveis():
    ...
    row = conn.execute('SELECT is_worker,subscribed FROM users WHERE username=?', (session['user'],)).fetchone()
    ...
    if not row['subscribed']:
        ...
        return jsonify({
            "blocked": True,
            "message": "Assine o plano PRO para acessar tarefas.",
            "tasks": []
        }), 403
...
def pegar_tarefa(tarefa_id):
    ...
    if not row['subscribed']:
        conn.close()
        return jsonify({"error": "Assine o plano PRO para trabalhar"}), 403
...
def verificar_tarefa(tarefa_id):
    conn = get_db()
    row_user = conn.execute('SELECT subscribed FROM users WHERE username=?', (session['user'],)).fetchone()
    if not row_user or not row_user['subscribed']:
        conn.close()
        return jsonify({"error": "Assine o plano PRO para enviar comprovante"}), 403
    ...
    registrar_movimentacao(conn, session['user'], 'RECOMPENSA_TAREFA', ganho, f'Recompensa da tarefa #{tarefa_id}', 'tarefas', tarefa_id)
    conn.commit()
    conn.close()
...
def solicitar_saque():
    ...
    registrar_movimentacao(conn, session['user'], 'SAQUE_SOLICITADO', -valor, f'Solicitação de saque para {carteira[:10]}...', 'saques', saque_id)
    conn.commit()
    conn.close()
    return jsonify({
...
def api_movimentacoes():
    conn = get_db()
    rows = conn.execute(
        'SELECT id,tipo,valor,descricao,ref_tabela,ref_id,criado_em FROM movimentacoes WHERE usuario=? ORDER BY criado_em DESC, id DESC LIMIT 100',
        (session['user'],)
    ).fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
...
def admin_aprovar_deposito(deposito_id):
    ...
    registrar_movimentacao(conn, deposito['usuario'], 'APORTE_APROVADO', float(deposito['valor']), f'Aporte aprovado pelo admin: TXID {deposito['txid']}', 'depositos', deposito_id)
    conn.execute(
        """UPDATE depositos
...
def admin_aprovar_saque(saque_id):
    ...
    conn.execute("UPDATE saques SET status='aprovado', aprovado_por=?, observacao=?, data_processamento=CURRENT_TIMESTAMP WHERE id=?", (session['user'], observacao, saque_id))
    bonus = 0
...
def admin_rejeitar_saque(saque_id):
    ...
    registrar_movimentacao(conn, saque['usuario'], 'SAQUE_ESTORNADO', float(saque['valor']), 'Saque rejeitado pelo admin e valor estornado', 'saques', saque_id)
    conn.execute("UPDATE saques SET status='rejeitado', aprovado_por=?, observacao=?, data_processamento=CURRENT_TIMESTAMP WHERE id=?", (session['user'], observacao, saque_id))
    conn.commit()
    conn.close()
